...
Tab **Route Plan** memakai filter MT dan pagination berbasis MT unik. Kontrol pagination ditempatkan tepat di bawah ringkasan **Solver Status / Gate Out / Dispatch Span** dan sebelum daftar/tabel route agar dispatcher menentukan kelompok MT sebelum membaca detail. Page dan pilihan `All MT`/satu nomor MT menjadi satu state bersama dengan card **Vehicle Multi-Trip Timeline**, sehingga kedua card selalu menampilkan kelompok MT yang sama. Kolom Product membaca nama canonical dari `master_product.product_name` (misalnya Bio Solar atau Pertamax); `product_id` tetap tersedia pada payload untuk lineage dan filter. Card **Operational KPI Groups** berada di tab **Simulation**, tepat di bawah **Simulation KPI**.
...
Saved Phase 6 Run (immutable)
    ...
    → CP-SAT bay eligibility, queue, loading, and gate-out schedule
    → iterative return/availability coordination
...
- Route dan bay mempunyai budget terpisah: `route_optimization_time_limit` dan `bay_optimization_time_limit`. Route budget dibagi antarputaran multi-trip; bay CP-SAT memakai `bay_cp_sat_workers` (default 8).
- CP-SAT bay scheduler menerapkan mode constraint untuk eligible product, actual occupancy/queue, no-overlap, dan batas gate-out depot/bay. Saat `serve_loading_order` SOFT, solver boleh memilih subset trip berdasarkan penalty per LO: trip yang dapat dipertahankan tetap menjadi hasil `PARTIAL`, sedangkan sisanya diberi `BAY_CONGESTION`. Trip yang baru kembali setelah depot tutup boleh menyelesaikan return tetapi trip lanjutannya tidak dapat loading/gate-out lagi. `BAY_PRODUCT_CONSTRAINT` hanya dipakai ketika tidak ada bay yang lolos eligibility struktural.
