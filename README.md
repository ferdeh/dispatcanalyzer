...
- full deployment-image regression terbaru lulus **126 tests**; focused Phase 7 lulus **45 tests**, termasuk reference time/timezone depot, asynchronous reservation, bay subset/timeout semantics, depot gate-out window yang tidak membatasi return MT, working-time sampai return, kronologi reroute, geometry depot-stop-depot, kelanjutan nomor trip setelah frozen plan, activation cost satu kali per MT, dan material gate-out change
...
Klik **Run Initial Optimization** selalu membuka dialog tanggal dan waktu referensi. Tanggal Initial harus sama dengan `operating_date` Job; waktu tersebut menjadi batas awal availability MT, Bay State Effective/queue, departure matrix, dan seluruh perhitungan waktu route. Klik **Re-Optimize Now** membuka dialog yang sama dengan tanggal dikunci ke tanggal Initial. Jam Re-optimize tidak boleh mundur dari run terakhir dan menjadi patokan `current time` untuk freeze window serta state bay. Timestamp pilihan dispatcher disimpan pada `optimization_run.optimization_reference_time`, solver metadata, dan audit state snapshot; jam server API tidak digunakan sebagai effective time operasional bay.
...
Saved Phase 6 Run (immutable)
    → Phase 7 LO operational state + soft warm start
    ...
    → post-bay working-time repair (failed LO → routing pool → alternative MT → global bay re-schedule)
...
- Phase 6 adalah warm start. Setiap business constraint Phase 7 mempunyai `enabled`, mode `HARD`/`SOFT`, dan nilai penalty yang disimpan di parameter profile. `HARD` wajib lolos dan mengabaikan penalty; `SOFT` boleh dilanggar dengan penalty; disabled tidak dienforce dan penalty efektifnya nol.
- `vehicle_working_time.limit_minutes` adalah satu-satunya sumber batas kerja MT. Durasi dihitung kumulatif sejak MT mulai digunakan, termasuk queue/loading, perjalanan dan service, sampai kembali ke depot; field lama `default_vehicle_working_time_minutes` sudah dihapus.
...
- Route assignment belum dianggap final sebelum waktu queue/loading/gate-out bay diterapkan. Jika pergeseran post-bay membuat sebuah MT melampaui hard `vehicle_working_time`, trip gagal dikeluarkan sementara, seluruh LO-nya dikembalikan ke routing pool, MT yang gagal diblokir untuk LO tersebut, lalu solver mencoba kandidat kompatibel lain memakai availability dan remaining working time aktual dari trip yang bertahan. Semua trip kemudian dijadwalkan ulang bersama-sama pada bay agar tidak terjadi overlap. `VEHICLE_TIME_EXHAUSTED` baru menjadi drop final setelah pool alternatif tidak lagi menghasilkan assignment feasible; habis budget dicatat terpisah sebagai `POST_BAY_REASSIGNMENT_TIMEOUT`, bukan bukti `INFEASIBLE`.
...
- `DONE`, `ONGOING`, dan near-term `PLANNED` di dalam `freeze_window_minutes` dibekukan. Jika satu LO frozen berada pada trip yang sama, seluruh trip dipertahankan sebagai unit execution yang konsisten.
...
- Actual user ETA/bay/queue state mengalahkan previous predicted state. Bay/queue state selalu disejajarkan ke timestamp Initial atau Reroute yang dimasukkan user. Reroute tidak menjalankan Phase 6 ulang.
