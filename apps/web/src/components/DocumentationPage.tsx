    ...
    topics: [
      {
        ...
        steps: [
          ...
          { title: "Buat baseline V1", text: "Klik Run Initial Optimization lalu isi tanggal dan waktu referensi pada popup. Tanggal harus sama dengan Operating Date Job. Pre-run menolak jika salah satu Planned ETA kosong atau lebih awal dari waktu optimasi; nilai yang sama persis tetap valid. Setelah lolos, API mengembalikan 202, Job menjadi CALCULATING, UI kembali ke Job Management, dan worker melanjutkan matrix/solver/persistence di background." },
          { title: "Update dan reroute", text: "Setelah Initial, isi kembali Planned ETA untuk seluruh MT karena input sebelumnya sudah di-reset. Ubah status LO atau bay queue aktual, lalu klik Re-Optimize Now; tanggal dikunci ke tanggal Initial dan waktu tidak boleh mundur dari run terakhir. Validasi Planned ETA yang sama berlaku sebelum reroute asynchronous membuat V2/V3." },
        ],
      },
      {
        id: "doc-phase7-card",
        ...
        cards: [
          ...
          { name: "MT Management", meaning: "Search, sort per header, pagination, capacity/compartment, Planned/System/Effective ETA, status, dan working time.", reading: "User ETA Override sudah dihapus. Planned ETA wajib untuk semua MT, menjadi satu-satunya availability input, dan di-reset setelah sukses. System ETA kosong sebelum V1; setelah run nilainya berasal dari calculated return trip ONGOING, atau Trip 1 bila belum ada trip ongoing." },
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Assignment route belum final sebelum queue/loading/gate-out bay diterapkan. Jika pergeseran post-bay melampaui hard MT Working Time, LO pada trip gagal dikembalikan ke routing pool dan MT tersebut diblokir untuk retry LO itu. Solver mencoba MT kompatibel lain memakai availability serta remaining working time aktual dari trip post-bay yang bertahan, lalu seluruh trip dijadwalkan ulang bersama pada bay. LO baru mendapat VEHICLE_TIME_EXHAUSTED setelah alternatif tidak feasible; budget retry yang habis diberi POST_BAY_REASSIGNMENT_TIMEOUT, bukan INFEASIBLE.",
        ...
        formulas: [
          "Sebelum V1: System ETA = null\nInput availability Initial/Reroute = Planned ETA Depot\nValid jika Planned ETA setiap MT >= optimization reference time\nSetelah Initial sukses: Planned ETA = null; System ETA = calculated return Trip 1\nSetelah Reroute sukses: Planned ETA = null; System ETA = calculated return trip ber-LO ONGOING, fallback calculated return Trip 1 bila tidak ada ongoing",
        ...
        examples: [
          ...
          { title: "Post-bay MT reassignment", text: "Trip awal MT01 feasible sebelum loading, tetapi queue bay membuat return melewati working time. System mengembalikan LO ke pool, mengecualikan MT01, menghitung sisa kerja MT02 dari jadwal bay aktual, mencoba MT02, lalu menjadwalkan ulang seluruh bay. LO hanya drop bila semua alternatif gagal; timeout retry dicatat sebagai timeout." },
