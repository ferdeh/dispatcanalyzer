    ...
    topics: [
      {
        ...
        steps: [
          ...
          { title: "Lengkapi current state", text: "Load MT canonical, isi Planned ETA Depot atau user override, lalu atur bay master, compatibility product, duration per compartment, occupancy aktual, dan queue fisik. Bay State Effective akan disejajarkan ke waktu Initial/Reroute yang dimasukkan user, bukan current time server." },
          { title: "Atur constraint dan validasi", text: "Buka Parameter → Constraint Settings. Aktifkan/nonaktifkan rule, pilih HARD atau SOFT, isi penalty untuk SOFT, atur MT Working Time, Route Time Limit, Bay Time Limit, dan Bay CP-SAT Workers. Default Working Time terpisah sudah dihapus. Kembali ke Job Overview dan klik Validate di card Optimization Flow; hasil Optimization Readiness tampil sebagai popup memakai draft constraint yang sedang aktif." },
          { title: "Buat baseline V1", text: "Klik Run Initial Optimization lalu isi tanggal dan waktu referensi pada popup. Tanggal harus sama dengan Operating Date Job. Setelah preflight, API mengembalikan 202, Job menjadi CALCULATING, UI kembali ke Job Management, dan worker melanjutkan matrix/solver/persistence di background. Daftar Job diperbarui otomatis sampai V1 selesai; kemudian review Route Plan, Simulation, Map, Cost, serta Dropped LO." },
          { title: "Update dan reroute", text: "Ubah status LO, MT ETA/status, atau bay queue aktual. Re-Optimize Now meminta waktu lagi, tetapi tanggal dikunci ke tanggal Initial. Waktu baru menjadi patokan freeze dan tidak boleh mundur dari run terakhir. Reroute juga asynchronous: UI kembali ke Job Management dengan status CALCULATING sementara worker membuat V2/V3 tanpa mengubah versi sebelumnya atau menjalankan Phase 6 ulang." },
        ],
      },
      {
        id: "doc-phase7-card",
        ...
        cards: [
          ...
          { name: "Job Overview", meaning: "Source run serta Optimization Flow untuk Validate, Initial, dan Re-optimize.", reading: "Validate berada tepat sebelum Run Initial Optimization dan membuka popup Optimization Readiness. Source Phase 6 dikunci setelah load; setiap optimize wajib memiliki reference time dalam timezone depot." },
          { name: "LO Management", meaning: "Search, sort per header, pagination, selection, Phase 6 assignment, current assignment, dan status.", reading: "Select-all hanya memilih LO pada halaman aktif. PLANNED dapat dioptimalkan; ONGOING/DONE serta freeze-window ditahan." },
          { name: "MT Management", meaning: "Search, sort per header, pagination, capacity/compartment, ETA, status, dan working time.", reading: "Edit tetap tersimpan pada draft meskipun row berpindah halaman. Prioritas ETA: user override → system return → planned initial." },
          ...
          { name: "Parameter", meaning: "Objective, solver, biaya, registry constraint HARD/SOFT/disabled, route/bay time limit, CP-SAT workers, penalty, serta Vehicle Activation Cost Rules.", reading: "Route dan bay memakai budget terpisah. MT Working Time menyimpan limit menit langsung pada rule. Priority activation memilih rule biaya yang menang; activation cost memengaruhi objective, bukan compatibility. Save membuat profile version dan setiap run menyalin effective snapshot + checksum." },
          { name: "Route Plan", meaning: "MT → Trip → sequence SPBU → LO/product/compartment/ETA, dengan filter dan pagination per MT.", reading: "Pagination berada di bawah ringkasan Solver Status/Gate Out/Dispatch Span dan sebelum tabel. Dropdown All MT/satu nomor MT serta halaman MT tersinkron dengan Vehicle Multi-Trip Timeline. Kolom Product menampilkan nama canonical seperti Bio Solar atau Pertamax." },
      ...
      },
      {
        ...
        paragraphs: [
          "OR-Tools Routing Solver menentukan assignment MT dan sequence SPBU dari matrix yang sudah dibangun. CP-SAT menangani compartment serta bay/loading/gate-out. Registry constraint menentukan apakah rule wajib dipenuhi, boleh dilanggar dengan penalty, atau tidak diterapkan. Google Routes hanya memberi distance/time/geometry dan tidak pernah menjadi optimizer atau dipanggil dari solver callback.",
          "Satu MT fisik dapat menjalankan Trip 1, kembali ke depot, masuk queue/loading lagi, lalu menjalankan Trip 2 selama availability, working time, compartment, bay, time window, dan gate-out depot masih feasible. Depot Operating Window membatasi loading dan gate-out, bukan waktu return: gate-out terakhir boleh tepat saat depot tutup dan MT boleh kembali sesudahnya selama MT Working Time serta SPBU Receiving Window masih terpenuhi. Route budget dibagi antarputaran agar Trip 1 tidak menghabiskan seluruh waktu pencarian. MT Working Time tetap dihitung sampai kembali ke depot. Initial/Re-optimize reference time menjadi current time MT, route, freeze, dan Bay State Effective.",
          "Bay CP-SAT memakai beberapa search worker dan budget bay sendiri. Serve Loading Order HARD mewajibkan semua trip memperoleh bay; mode SOFT mengizinkan subset dengan penalty per LO. Subset feasible menghasilkan PARTIAL dan trip eligible yang tidak tertampung diberi BAY_CONGESTION. BAY_PRODUCT_CONSTRAINT hanya berarti tidak ada bay yang lolos eligibility struktural. UNKNOWN/TIMEOUT dipertahankan dan tidak disamakan dengan INFEASIBLE.",
        ...
        examples: [
          ...
          { title: "Bay congestion partial", text: "Dua trip sama-sama eligible tetapi sisa jam bay hanya cukup untuk satu trip. Dengan Serve Loading Order SOFT, CP-SAT mempertahankan trip dengan objective terbaik, menghasilkan PARTIAL, lalu mencatat LO trip lain sebagai BAY_CONGESTION beserta unserved penalty." },
